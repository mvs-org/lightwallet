...
export default {
  // 使用千位分隔符格式化大数字
  formatNumber(numString, decimals) {
    // 判断是不是数字，非数字返回字符串"-"
    if (!REGEXP.isNum(numString)) {
      return '-'
    }
    ...
    let res = numArray.join('.')
    return res
  },
  // 数字补零（呈现出01、002、0003之类的效果）
  // numString: 需要补的数字
  // bits: 总位数
  paddingZero(numString, bits) {
  ...
  encryptKey(str, password) {
    ...
    let result = cryptoJS.enc.Base64.stringify(cipherText);
    //console.log("after base64: " + result);
    return result;
  },
  // 私钥解密
  decryptKey(str, password) {
    //console.log("encrypt:" + str);
    //console.log("password:" + password);
    password = cryptoJS.enc.Utf8.parse(password);
  ...
  // 下载私钥文件
  downloadKeyFile(filename, cipherText, net) {
    // 构造对象
    let jsonInfo = {
  ...
  // 格式化时间间距
  // 返回一个已经格式化的字符串
  formatTimeDistance(distance, isLater = false) {
    // 秒数
    ...
    let hourSec = 3600;
    let daySec = 86400;
    isLater = isLater ? "Later" : "";
    if (sec < minSec) {
      str = _vm.$t('global.timeDistanceSec' + isLater, { s: sec })
      return str
    }
    if (sec >= minSec && sec < hourSec) {
      ...
      //let s = sec % minSec
      str = _vm.$t('global.timeDistanceMin' + isLater, { m })
      return str
    }
    if (sec >= hourSec && sec < daySec) {
      ...
      str = _vm.$t('global.timeDistanceHours' + isLater, { h })
      return str
    }
    if (sec >= daySec) {
      let d = parseInt(sec / daySec)
      str = _vm.$t('global.timeDistanceDays' + isLater, { d })
      return str;
    }
  },
  // 得到最近过去七天的日期
  // 返回一个字符串数组
  getPastSevenDate(includeToday = false) {
   ...
   */
  sortArray(list, property, direction) {
    // 拷贝原数组
    let nList = []
    for (let i in list) {
      nList.push(list[i])
   ...
   * @param {*显示少数点位数} fixed 
   */
  formatNumeral(value, symbols, fixed = 2, trimZero = false) {
   ...
   */
  formatToken(value, numeralSymbols = [], fixed = 2, tokenSymbol = DATA.TOKEN_SYMBOL, tokenDecimal = DATA.TOKEN_DECIMAL) {
    let trimZero = true;//原来的全部trim

    let bn = BigNumber(value);
    value = bn.div(BigNumber(10).pow(tokenDecimal)).toFixed(fixed);
  ...
  formatTokenWithOptions(value, opts) {
    opts = opts || {};
   ...
   * @param {*} tokenDecimal 
   */
  toUnit(value, tokenDecimal = DATA.TOKEN_DECIMAL) {
    let bn = BigNumber(value);
    bn = bn.times(BigNumber(10).pow(tokenDecimal));
    return bn.toFixed(0);
  },
  /**
   * 把 基础单位数值 转换成 常用单位数值
   ...
   * @param {*} tokenDecimal 
   */
  toToken(value, tokenDecimal = DATA.TOKEN_DECIMAL) {
    let bn = BigNumber(value);
    bn = bn.div(BigNumber(10).pow(tokenDecimal));
