...
import { AES, enc, mode, SHA256 } from 'crypto-js'

import { DnaUtilRegexProvider } from './dna-util-regex';
...
export class DnaUtilUtilProvider {
    ...
    static encryptKey(str, password) {
        ...
        password = enc.Utf8.parse(password);
        ...
        let cipherText = AES.encrypt(str, key, {
            iv: iv,
            mode: mode.CBC
        }).ciphertext;
        //console.log("after encrypt: " + cipherText);

        return enc.Base64.stringify(cipherText);
    }
    ...
    static decryptKey(str, password) {
        ...
        let cipherText = enc.Base64.parse(str);
        cipherText = enc.Base64.stringify(cipherText);
        //cipherText = enc.Utf8.parse(cipherText);
        // 格式化密文
        // console.log(`before decrypt: ${cipherText}, key: ${key}, iv: ${iv}`);

        let objText
        try {
            objText = AES.decrypt(cipherText, key, {
                iv: iv,
                mode: mode.CBC
